defmodule CinderWeb.LibraryAdoptionLiveTest do
  ...
  # #513 review finding (PR #567): a :multi_track audiobook candidate must present ONE truthful
  # control, not the Preferred/All formats pair (which falsely implies a choice exists when both
  # values adopt the identical complete track set).
  test "a :multi_track audiobook candidate offers a single truthful 'Adopt all tracks' control",
       %{conn: conn} do
    track1_path = "/readarr/two-track/01.mp3"
    track2_path = "/readarr/two-track/02.mp3"

    saved_audiobooks_root = Application.get_env(:cinder, :audiobooks_library_path)
    Application.put_env(:cinder, :audiobooks_library_path, "/readarr")

    on_exit(fn ->
      if saved_audiobooks_root,
        do: Application.put_env(:cinder, :audiobooks_library_path, saved_audiobooks_root),
    ...
    stub(Cinder.Library.ReadarrMigrationSourceMock, :snapshot, fn ->
      {:ok,
       %{
         ...
         editions: [],
         files: [
           %{provider_id: 1, kind: :book, path: track1_path, size: 10, work_id: 1, format: "mp3"},
           %{provider_id: 2, kind: :book, path: track2_path, size: 10, work_id: 1, format: "mp3"}
         ],
         profiles: [],
         roots: []
       }}
    ...
    stub(Cinder.Books.PrimaryMetadataMock, :search, fn _query ->
      {:ok,
       [
         %{
           provider: :openlibrary,
           foreign_id: "ol-track-1",
           title: "Two Track Audiobook",
           contributors: [%{foreign_id: "a1", name: "Track Author", role: "author"}],
           contributors_incomplete: false,
           first_published_year: nil,
           edition_count: 1
         }
    ...
    stub(Cinder.Books.PrimaryMetadataMock, :get_work, fn "ol-track-1" ->
      {:ok,
       %{
         provider: :openlibrary,
         foreign_id: "ol-track-1",
    ...
    html = render(view)
    assert html =~ "Adopt all tracks"
    refute has_element?(view, "#migration-decision-candidates input[value=preferred]")

  ...
  subagents: [
    ...
    %{
      ...
      **LEGITIMATE — do NOT flag (these are sanctioned direct writes in `catalog.ex`):**
      ...
      - TMDB refresh reconcile (`refresh_changeset` preserves monitored/file_path/grab_id/counters; all gated by the in-flight-grab guard ~L2658): `update_series_row/2` (~L2779), `ensure_season/3` (~L2901), `park_episode/1` (~L2875), `finalize_or_restore/4` (~L2929), `insert_episode/2` (~L2958).
      ...
      **The secret set is closed: 6 keys**, all under `@base_config_fields` with `secret: true`: `tmdb_token`, `prowlarr_api_key`, `qbittorrent_password`, `sabnzbd_api_key`, `jellyfin_api_key`, `plex_token`. `@secret_keys` (`settings.ex:~245`) + `secret?/1` (~L1066) is the single authority for encrypt-on-write and withhold-from-form.

      **Key locations:** `settings.ex` `@base_config_fields` (~L24-243), `upsert/2` (~L1068, `if secret?(key), do: Base.encode64(Cinder.Vault.encrypt!(value)), else: value`), `decoded/1` (~L1030-1046, the `is_binary(plaintext)` guard at ~L1039 catches Cloak's `{:ok, :error}` GCM-auth failure + `rescue` at ~L1044; both log key-only and return `:error`), `form_state/0` (~L394, builds `values` ONLY for `not f.secret` fields — secrets surface only as boolean membership `secrets_set`/`secrets_from_env`), `plan_config/3` secret clause (~L1131, blank-keeps + explicit-Clear), `load_into_env/0` (~L684, rescue+catch degrade to env bootstrap, never bricks boot); `settings_components.ex` `setting_field/1` (~L400, secret branch renders `<input type="password" value="">` hardcoded empty at ~L420 + a `clear_<key>` checkbox at ~L431), `secret_placeholder/1` (~L503, masked hint only, reads `form.secrets_set`/`form.secrets_from_env`); `settings_live.ex` (admin-gated, assigns only `Settings.form_state()`); `vault.ex` (~L8, `use Cloak.Vault`); `config/runtime.exs` (~L167, key = `:crypto.hash(:sha256, secret_key_base <> "cinder.vault")`); `application.ex` (~L24-26, Vault started before the settings loader, after PubSub).

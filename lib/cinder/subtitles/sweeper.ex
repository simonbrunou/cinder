defmodule Cinder.Subtitles.Sweeper do
  ...
  wanted-language sidecar via `Cinder.Subtitles.fetch_missing/3`, halting the tick once a download
    ...
    units =
      Enum.map(Catalog.list_available_movies_with_file(), fn m ->
        ...
        Enum.map(Catalog.list_episodes_with_file(), fn ep ->
          {"episode #{ep.id}", fn -> Subtitles.episode_criteria(ep) end, ep.file_path, :tv}
        end)
  ...
  # next tick resumes.
  defp fetch_unit({label, criteria_fun, path, kind}, _acc) do
    case isolate(label, fn -> Subtitles.fetch_missing(criteria_fun.(), path, kind) end) do

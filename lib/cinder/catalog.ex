defmodule Cinder.Catalog do
  ...
  *native* code of a different episode in the series. Collisions are the case the operator must
  ...
  # display-only data).
  defp merge_localizations(stored, new) do
  ...
  defp fetch_seasons(tmdb_id, season_stubs, locale \\ Locales.canonical()) do
    result =
  ...
  defp put_episode_localizations(tmdb_id, seasons) do
    titles =
  ...
  # changed the group in between (`sync_scene_coordinates_if_current/3`).
  ...
  defp scene_group_detail(absolute_groups, group_id) do
    case Enum.find(absolute_groups, &(&1.id == group_id)) do
      nil -> fetch_scene_group_detail(group_id)
      detail -> detail
    end
  end
  ...
  defp sync_series_identity(series, seasons, identity) do
    ...
    with {:ok, _} <-
         ...
         :ok <- sync_scene_coordinates_if_current(series, identity, episode_lookup),
  ...
  # `set_scene_numbering_group/3` and `fetch_series_identity/2`), never inside one: TMDB is a
  ...
  **Non-raising**: runs inside the poller's isolated park path, where a raise would re-fire
  ...
  def refresh_series(%Series{} = series) do
    with {:ok, info} <- tmdb().get_series(series.tmdb_id),
         ...
         seasons = put_episode_localizations(series.tmdb_id, seasons),
         {:ok, identity} <-
  ...
  # Shared by refresh_series/1 and set_scene_numbering_group/3: a `Repo.transaction` result whose
  # ok value is the updated `%Series{}` broadcasts once on the "series" topic and passes through;
  # an error passes through untouched. `updated.id` always equals the series being written in
  # both callers, so there's no need for a separate id argument.
  ...
  defp update_series_row(series, info) do
    changeset =
      Series.refresh_changeset(series, %{
        ...
        overview: Map.get(info, :overview),
        # Merge-over, not replace (see merge_localizations/2): a transient/partial translations
        # payload must never strip last-synced localizations.
        localizations:
          merge_localizations(series.localizations, Map.get(info, :localizations, %{})),
        genres: Map.get(info, :genres),
        vote_average: Map.get(info, :vote_average),
        first_air_date: Map.get(info, :first_air_date)
      })
  ...
  defp finalize_or_restore(parked_ep, original, season_id, fe) do
    changeset =
      Episode.refresh_changeset(parked_ep, %{
        ...
        title: fe.title,
        # Merge-over (see merge_localizations/2): a failed non-canonical season fetch yields %{}
        # here, and replacing would erase the last-synced titles until the next successful refresh.
        localizations: merge_localizations(parked_ep.localizations, fe.localizations),

defmodule Cinder.Requests.WatchlistSync do
  ...
  Dedupe is "this user already has a request row for this tmdb_id" (any status), so a title is
  requested at most once however many ticks it sits on the watchlist. Removing a title from the
  watchlist does nothing at all: no un-requesting, no deletion.
  ...
  defp request_new(user, entries) do
    already_requested = requested_tmdb_ids(user)

    for %{type: "movie", tmdb_id: tmdb_id} = entry <- entries,
        not MapSet.member?(already_requested, tmdb_id) do
  ...
  defp request_one(user, entry) do
    ...
    case Requests.create_request(user, attrs) do
      ...
      # Quota reached, a racing manual request, a TMDB hiccup: the entry stays on the watchlist,
      # so the next tick retries it once the cause clears.
      {:error, reason} ->
        Logger.info(

defmodule Cinder.Library.MigrationAdoption do
  ...
  def adopt(source, commands) when source in [:radarr, :sonarr] and is_list(commands) do
    case preview(source) do
      {:ok, preview} ->
        by_key = Map.new(preview.candidates, &{&1.key, &1})
        {selected, missing} = selected_candidates(commands, by_key)

        %{adopted: 0, skipped: missing, failures: []}
        |> adopt_selected(source, selected)

      {:error, reason} ->
        %{adopted: 0, skipped: length(commands), failures: [%{reason: reason}]}
    end
  end

  def adopt(_source, _commands), do: %{adopted: 0, skipped: 0, failures: []}
  ...
  defp plan_episodes(snapshot, reconciled, lookups, managed) do
    ...
    merge_n_to_one(grouped, lookups, managed) ++ unlinked
  end
  ...
  defp episode_group_candidate(
    ...
    base = %{
      ...
      series_episodes:
        resolved
        |> Map.values()
        |> Enum.filter(&(&1.series_id == List.first(series_ids)))
    }
  ...
  defp put_episode_group_status(base, series_ids, series_info, reasons, identities, file, managed) do
    {status, reason} =
      cond do
        ...
        true -> episode_file_status(base, managed)
      end
  ...
  defp episode_file_status(candidate, managed) do
    with %Series{} = existing <- existing_series(candidate),
         {:ok, targets} <- episode_targets(existing, candidate.episodes) do
  ...
  defp existing_series(%{series: %{tmdb_id: tmdb_id}}),
    do: Catalog.get_series_by_tmdb_id(tmdb_id)

  defp episode_targets(series, identities) do
    by_tmdb =
      series.id
      |> Catalog.get_series_with_tree()
      |> episode_tree()
      |> Map.new(fn {tmdb_id, episode, _season} -> {tmdb_id, episode} end)

    targets = Enum.map(identities, &Map.get(by_tmdb, &1.tmdb_episode_id))
  ...
  defp merge_n_to_one(candidates, lookups, managed) do
    ...
    merged =
      Enum.map(collisions, fn {tmdb_id, keys} ->
        ...
        n_to_one_candidate(tmdb_id, members, lookups, managed)
      end)
  ...
  defp n_to_one_candidate(tmdb_id, members, lookups, managed) do
    all_only_target? =
    ...
    cond do
      ...
      true ->
        ...
        {status, reason} = n_to_one_status(primary, extras, managed)
  ...
  defp n_to_one_status(primary, extras, managed) do
    case existing_series(primary) do
      %Series{} = series ->
        case episode_targets(series, primary.episodes) do
  ...
  defp selected_candidates(commands, by_key) do
    Enum.reduce(commands, {[], 0}, fn command, {selected, missing} ->
      key = command[:key] || command["key"]
      choice = command[:choice] || command["choice"]

      case Map.get(by_key, key) do
        %{status: :ready} = candidate ->
          {[{candidate, nil} | selected], missing}

        %{status: :needs_decision} = candidate when choice in [:fold, "fold", :part, "part"] ->
          {[{candidate, normalize_choice(choice)} | selected], missing}

        _unavailable ->
          {selected, missing + 1}
      end
    end)
    |> then(fn {selected, missing} -> {Enum.reverse(selected), missing} end)
  end
  ...
  defp persist_series_items(summary, items, series, existing?) do
    ...
    with {:ok, actions} <- series_actions(items, episode_targets),
      ...
      Map.update!(summary, :adopted, &(&1 + length(items)))
    else

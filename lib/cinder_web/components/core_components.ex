defmodule CinderWeb.CoreComponents do
  ...
  @doc """
  Renders the Plex-branded sign-in / link button — Plex gold with the Plex wordmark.
  `label` is the already-translated text; `rest` carries `href`/`navigate`.
  """
  attr :label, :string, required: true
  attr :class, :any, default: nil
  attr :rest, :global, include: ~w(href navigate patch)

  def plex_button(assigns) do
    ~H"""
    <.link
      class={["btn w-full gap-2 border-none bg-[#e5a00d] text-black hover:bg-[#cc7b19]", @class]}
      {@rest}
    >
      <svg viewBox="0 5 24 14" fill="currentColor" class="h-4 w-auto shrink-0" aria-hidden="true">

defmodule CinderWeb.UserAuth do
  @moduledoc false
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      CinderWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  # Renews the session ID and erases the whole session to avoid fixation attacks.
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      CinderWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule CinderWeb.PageLive do
        use CinderWeb, :live_view

        on_mount {CinderWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{CinderWeb.UserAuth, :require_authenticated}] do
